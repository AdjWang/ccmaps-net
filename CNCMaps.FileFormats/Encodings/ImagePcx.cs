...
namespace CNCMaps.FileFormats.Encodings {
	public static class PcxGet {
		public static Bitmap Convert(string pcxFilePath) {
			try {
				FileInfo fi = new FileInfo(pcxFilePath);
				string name = fi.Name.Split('.')[0];
				ImagePcx rmr = new ImagePcx(pcxFilePath);
				return rmr.PcxImage;
			}
			catch (Exception ee) {
				Logger.Error("ImagePcx:" + ee.Message);
				return null;
			}
		}
